...
### MSYS2
Compiling with MSYS2 requires no additional steps(You must first install the required development packages); simply execute the following command:
./build-mingw-sdl2
...
### Language File
This fork includes two completed language files: `en_US.lng` in the `en` folder and `zh_TW.lng` in the `zh` folder. Please copy them to the `languages` folder under the DOSBox-X installation directory. For unknown reasons, programs compiled with MinGW cannot properly interpret the language files. Until this issue is resolved, please use Visual Studio to build dosbox-x.

If you want to create your own language file, please refer to the `README.md` file in the `lang` folder.
