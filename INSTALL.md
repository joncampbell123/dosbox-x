...
If you prefer to use one of the portable packages, please select the zip package you want to download for your platform and unzip, then you will find various folders or subdirectories, which are some supported targets. For Visual Studio builds, these correspond to Win32 (x86), Win64 (x64), ARM32 and ARM64 (either SDL1 or SDL2 version), which are the build platforms. For MinGW builds, the targets are the MinGW SDL1 build (mingw) and the MinGW SDL2 build (mingw-sdl2). Go to a target folder for your platform and run dosbox-x.exe inside it, then DOSBox-X will be launched and ready to be used. Unlike the Windows installer version however, there are no documentations, language files, OpenGL shaders, TTF fonts etc included in these packages, and you may not see all such packages for some DOSBox-X versions. **It is highly recommended to use the Windows installers for more complete installations.**
