...
{
	if(key.keysym.sym == 0 || (!SDL_IM_Composition() && (key.keysym.sym == 0x08 || key.keysym.sym == 0x09 || key.keysym.sym >= 0x20 && key.keysym.sym <= 0x7D || key.keysym.sym >= 0x111 && key.keysym.sym <= 0x114))) {
		// BS, <-, ->
		return true;
	}
