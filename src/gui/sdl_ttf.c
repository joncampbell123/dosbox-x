...
static FT_Error Load_Glyph(TTF_Font *font, Uint32 idx, c_glyph *cached, int want)
{
    ...
    if ((want & CACHED_METRICS) && !(cached->stored & CACHED_METRICS)) {
        ...
        if (TTF_HANDLE_STYLE_ITALIC(font) && FT_IS_SCALABLE(face)) {
            ...
            cached->maxx += bump;
#else
            cached->maxx += (int)ceil(GLYPH_ITALICS * font->height);
#endif
        }
                ...
                const char *text, SDL_Color fg, SDL_Color bg, unsigned int expect_width)
{
    ...
    if (utf8) {
        LATIN1_to_UTF8(text, utf8);
        surface = TTF_RenderUTF8_Shaded(font, (char *)utf8, fg, bg, expect_width);
        SDL_stack_free(utf8);
    } else {
        SDL_OutOfMemory();
    }
                     ...
                     SDL_Color bg)
{
    ...
    ucs2[1] = 0;
    UCS2_to_UTF8(ucs2, utf8);
    return TTF_RenderUTF8_Shaded(font, (char *)utf8, fg, bg, 0);
}
