...
static int kvm_fd = -1;
static bool kvm_avail = false;
static unsigned int kvm_vcpu_size = 0;
...
#define CPU_TRAP_DECODER	CPU_Core_KVM_Trap_Run
...
void CPU_Core_KVM_Init(void) {
	...
	if (kvm_fd < 0) {
		...
		if (r <= 0 || r > (1*1024*1024)) {
			close(kvm_fd);
			kvm_fd = -1;
			return;
		}
		kvm_vcpu_size = (unsigned int)r;

		kvm_avail = true;
		LOG_MSG("Linux KVM API is available (x86). VCPU mmap size=%u",(unsigned int)kvm_vcpu_size);
	}
}
