...
extern const char *prefix_local;
...
bool read_directory_first(dir_information* dirp, char* entry_name, char* entry_sname, bool& is_directory) {	
	...
	} while ((hidenonrep && CodePageHostToGuest(dentry->d_name)==NULL) || (ignorespecial && (strlen(dentry->d_name) > prefix_lengh+5) && strncmp(dentry->d_name,prefix_local,prefix_lengh) == 0));
