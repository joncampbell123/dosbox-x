...
This enables an emulated LPT1 port on IO base address 0x378. Most DOS applications expect a printer on LPT1 port.
...
This enables an emulated LPT3 port.
...
At a minimum it needs ``courier.ttf``.
If no TTF font can be found then DOSBox-X will print with the internal TTF font as used by the TrueType font output.
...
This assumes you PC still has a parallel printer port integrated on the systemboard, or a legacy ISA printer adapter.
