...
== Mouse support
By default Windows 3.x will find a PS/2 mouse when installed in DOSBox-X.
This mouse support has an important limitation, namely you need to lock the mouse to the session to interact with Windows 3.x and need to unlock it to go outside DOSBox-X.

DOSBox-X 0.83.24 or later however also supports the VMware mouse protocol, and there is a 3rd party link:https://github.com/NattyNarwhal/vmwmouse[VMware mouse driver for Windows 3.x] written by NattyNarwhal.
