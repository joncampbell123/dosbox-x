...
|===
Windows 2.x was available for both Retail and via OEM.
A limited runtime version was also included with some applications.
...
As such it has a very small selection of drivers, and for instance no driver for a PS/2 mouse.
