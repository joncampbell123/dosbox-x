...
ci_verify_version() {
    ...
    case "$PNG_LIBPNG_VER_STRING" in
        "$my_expect"*) ci_info "matched: \$PNG_LIBPNG_VER_STRING == $my_expect*" ;;
    ...
    case "$PNG_LIBPNG_VER_BUILD" in
        [01]) ci_info "matched: \$PNG_LIBPNG_VER_BUILD == [01]" ;;
