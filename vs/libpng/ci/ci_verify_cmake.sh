...
ci_build() {
    ...
    [ -n "$CI_CMAKE_TOOLCHAIN_FILE" ] && {
        all_cmake_vars="${all_cmake_vars}-DCMAKE_TOOLCHAIN_FILE=$CI_CMAKE_TOOLCHAIN_FILE
"
    }
    [ -n "$CI_CC" ] && {
        all_cmake_vars="${all_cmake_vars}-DCMAKE_C_COMPILER=$CI_CC
"
    }
    [[ $CI_CC_FLAGS || $CI_SANITIZERS ]] && {
        [ -n "$CI_SANITIZERS" ] && CI_CC_FLAGS="$CI_CC_FLAGS${CI_CC_FLAGS:+" "}-fsanitize=$CI_SANITIZERS"
        all_cmake_vars="${all_cmake_vars}-DCMAKE_C_FLAGS=$CI_CC_FLAGS
"
    }
    [ -n "$CI_AR" ] && {
        all_cmake_vars="${all_cmake_vars}-DCMAKE_AR=$CI_AR
"
    }
    [ -n "$CI_RANLIB" ] && {
        all_cmake_vars="${all_cmake_vars}-DCMAKE_RANLIB=$CI_RANLIB
"
    }
    all_cmake_vars="${all_cmake_vars}-DCMAKE_BUILD_TYPE=$CI_CMAKE_BUILD_TYPE
"
    all_cmake_vars="${all_cmake_vars}-DCMAKE_VERBOSE_MAKEFILE=ON
"
    all_cmake_vars="${all_cmake_vars}$CI_CMAKE_VARS
"
    all_cmake_build_flags="${all_cmake_build_flags}$CI_CMAKE_BUILD_FLAGS
"
    all_ctest_flags="${all_ctest_flags}$CI_CTEST_FLAGS
"
    # And... build!
